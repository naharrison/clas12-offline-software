 ...
 *
 */
public final class RECEventBank {
    ...
    public RECEventBank(DataBank dbank, int irow) {
    ...
    static public List<RECEventBank> getRECEventBanks(DataBank dataBank) {
        List<RECEventBank> banks = new ArrayList<>();
        for (int irow = 0; irow < dataBank.rows(); irow++) {
            banks.add(new RECEventBank(dataBank, irow));
        }
        return banks;
    }
}
