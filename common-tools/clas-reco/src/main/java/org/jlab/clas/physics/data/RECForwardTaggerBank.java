 ...
 *
 */
public final class RECForwardTaggerBank {
    ...
    static public List<RECForwardTaggerBank> getRECForwardTaggerBanks(DataBank dataBank) {
        List<RECForwardTaggerBank> banks = new ArrayList<>();
        for (int irow = 0; irow < dataBank.rows(); irow++) {
            banks.add(new RECForwardTaggerBank(dataBank, irow));
        }
        return banks;
    }
}
