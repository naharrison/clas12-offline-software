 ...
 *
 */
public final class RECCalorimeterBank {
    ...
    public RECCalorimeterBank(DataBank dbank, int irow) {
    ...
    static public List<RECCalorimeterBank> getRECCalorimeterBanks(DataBank dataBank) {
        List<RECCalorimeterBank> banks = new ArrayList<>();
        for (int irow = 0; irow < dataBank.rows(); irow++) {
            banks.add(new RECCalorimeterBank(dataBank, irow));
        }
        return banks;
    }
}
