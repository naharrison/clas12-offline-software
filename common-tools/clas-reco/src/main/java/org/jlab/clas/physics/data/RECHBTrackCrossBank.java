 ...
 *
 */
public final class RECHBTrackCrossBank {
    ...
    public RECHBTrackCrossBank(DataBank dbank, int irow) {
    ...
    static public List<RECHBTrackCrossBank> getRECHBTrackCrossBanks(DataBank dataBank) {
        List<RECHBTrackCrossBank> banks = new ArrayList<>();
        for (int irow = 0; irow < dataBank.rows(); irow++) {
            banks.add(new RECHBTrackCrossBank(dataBank, irow));
        }
        return banks;
    }
}
