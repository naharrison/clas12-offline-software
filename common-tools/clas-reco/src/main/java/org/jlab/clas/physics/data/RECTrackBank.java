 ...
 *
 */
public final class RECTrackBank {
    ...
    public RECTrackBank(DataBank dbank, int irow) {
    ...
    static public List<RECTrackBank> getRECTrackBanks(DataBank dataBank) {
        List<RECTrackBank> banks = new ArrayList<>();
        for (int irow = 0; irow < dataBank.rows(); irow++) {
            banks.add(new RECTrackBank(dataBank, irow));
        }
        return banks;
    }
}
